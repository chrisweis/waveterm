...
- **Workspace sidebar + emoji workspace icons** — an opt-in left pane that replaces the workspace-switcher button, plus the ability to use any emoji as a workspace icon. ("Sidebar" follows Wave's own naming for the widgets strip — `widgetsSidebarVisibleAtom`, `layout:widgetsvisible`.)
  ...
  - **Pinned-first still applies until you drag something.** `wcore.ListWorkspaces` sorts pinned-first, and with no saved order that backend sequence is passed through untouched. Once a manual order exists it wins outright and pinning becomes a marker only. Ids the saved order has never seen (workspaces created since the last drag) keep their backend order and land at the end rather than jumping to the top.
