...
- **Recent-activity glow** — opt-in via **View → Recent Activity Glow** or `app:activityglow: true`. Tints recently-used tabs and workspaces so you can see where you were last working; the tint fades as they go unused. Off by default.
  ...
  - **`staticTabId` never changes — key activity recording on `reinitVersion`.** This one shipped broken and was invisible in testing. `atoms.staticTabId` is `atom(initOpts.tabId)` (`global-atoms.ts`), a constant for the life of the renderer, and every Wave tab has its own webview. Re-activating an existing tab goes through `initWaveWrap` → `reinitWave()`, which bumps `reinitVersion` but does **not** remount the tree or change `staticTabId`. An effect keyed only on `staticTabId` therefore fires once per webview creation and never again, so the glow ranked tabs by view-creation order rather than by last use — precisely inverting the feature. It still showed a plausible-looking gradient, which is why nobody spotted it by eye. The atom is named "static" and means it.
  - **Recording is frontend-side**, in a `useEffect` in `workspace.tsx` keyed on the active tab id and `reinitVersion`, writing `activity:score` + `activity:ts` meta through the existing `SetMetaCommand`. Deliberately not a new backend hook in `wcore.SetActiveTab`: activity only happens while a frontend is running, `SetMetaCommand` already propagates correctly (see the WaveObj-update note below), and this keeps the Go diff down to two meta keys and one setting.
...
- **Longer tab names, and tabs that size to them** — two related changes to the **top** tab bar. (The left tab bar is unaffected: its tabs are full-width rows in a fixed-width column, so there is nothing to size to, and it never had a name-length cap in the first place.)
  - **The 14-character cap is now 48.** It lived as `MaxTabNameLength` _inside_ the `TabV` component body, with the same 14 duplicated as a bare literal in the keydown guard, so the two could drift. Hoisted to a module const and both uses now reference it. The cap is real, not cosmetic: it truncates the displayed name _and_ seeds the rename box, so the old value silently shortened names on save.
  - **`tab:autosize`** (View → **Size Tabs To Their Names**, default false) sizes each tab to its own name between 70px and 320px, instead of giving every tab an identical width. Off, behaviour is byte-for-byte what it was.
  - **Uniform width was baked into four places**, not one. The layout pass positioned tabs by `index * width`; the drag hit-test compared against `dragStartPositions[i]` using a single width; the reflow during a drag repositioned by index; and the wrapper width and new-tab scroll target were both `count * width`. All now walk a running offset, or a prefix sum via `getOffsetForIndex`.
  - **Widths are keyed by tab id, not index.** A drag splices `tabIds` in place, so an index-keyed width array would follow the _slot_ rather than the tab, and every tab past the drop point would lay out at the wrong width. `dragStartOrderRef` separately records the order as of the last `saveTabsPosition`, because `dragStartPositions` describes the pre-drag layout and pairing a saved position with a reordered tab's width drifts the hit-test.
  - **Measuring the natural width needs a temporary reflow.** `.name` is absolutely positioned and stretched across the tab, so it contributes no intrinsic width and setting the tab to `max-content` measures nothing. `measureNaturalTabWidth` widens `.name` to `max-content` for the length of one `getBoundingClientRect()` and restores it.
  - Touches `pkg/wconfig/settingsconfig.go`, `emain/emain-menu.ts`, `docs/docs/config.mdx`, `frontend/app/tab/tab.tsx`, `tabbar.tsx`, `tabbarenv.ts`.

- **Selection is a hue, recency is a luminance** — a deliberate split, made after the glow above turned out to make the active tab hard to find. Both signals had been painted as the same translucent white wash, and the glow ran to alpha 0.22 against the active tab's 0.10, so a recently-used _inactive_ tab was over twice as bright as the selected one. Brightness alone cannot carry both: recency is comparative and ambient, selection is categorical and absolute.
  - Active now reads as colour: the top tab bar and the vertical tab bar use an accent-tinted fill with a hairline accent edge; the workspace sidebar and switcher tint with the **workspace's own colour**, tying identity to selection. The hairline is an `inset box-shadow`, not a border, so activating a tab shifts nothing.
  - **The active item never draws a glow.** It is already the most salient thing on screen and the second wash only muddied it. Suppressed at each of the four render sites, not in the model, so `computeActivityAlphas` stays a pure function of recency.
  - If you raise `MaxAlpha` in `activityglow.ts`, check it against the active treatments — that ratio is the whole point.
- **Layout traps found in review** (all fixed; listed because they will recur):
  - **Never map `tabRefs[i]` to `tabIds[i]`.** `handleMouseMove` splices `tabIds` in place mid-drag while `tabRefs` keeps its original order, so a relayout during a drag writes each tab's width and offset onto a different tab. Resolve elements by `dataset.tabId`. The same stale-index thinking caused two earlier drag bugs; six of thirteen review findings were this one shape.
  - **React owns `className`; it does not own `data-` attributes.** `.name`'s class is re-rendered on every rename (the `focused` flag), so a class added imperatively from the layout pass is silently dropped. The overflow marker is `data-overflowing` for exactly this reason.
  - **Blur and Escape fire no `input` event.** Both set `innerText` programmatically, and the rename round-trips through an RPC that never re-renders `TabBar`, so nothing unwinds the rename-expand. `onBlur` on the tabs wrapper (which bubbles, unlike `blur`) is what ends it, deferred a frame so `.focused` is gone before widths are recomputed.
  - **The sidebar must carry its own "Save workspace".** `wcore.ListWorkspaces` filters out workspaces with no name/icon/colour and the sidebar replaces the switcher outright, so without it a fresh window sits on an unnamed workspace that appears in no list and cannot be named.
