...
import { WorkspaceRailModel } from "./workspacerail-model";
import "./workspacerail.scss";
import { WorkspaceRailEnv } from "./workspacerailenv";
...
const WorkspaceRailItem = memo(({ entry, compact, isCurrent, onSelect, onDeleteWorkspace }: WorkspaceRailItemProps) => {
    const env = useWaveEnv<WorkspaceRailEnv>();
    ...
    const applyEdit = useCallback((next: Workspace) => {
        ...
        fireAndForget(() => env.services.workspace.UpdateWorkspace(next.oid, next.name, next.icon, next.color, false));
    }, []);
        ...
        <Popover placement="right-start" onDismiss={() => setDraft(null)}>
            <PopoverButton
                ...
                className={cn(
                    ...
                    "workspace-rail-edit-btn ghost grey",
            ...
            <PopoverContent className="workspace-rail-editor-popover">
                <WorkspaceEditor
    ...
    return (
        <Tooltip content={workspace.name} placement="right" openDelay={0} hideOnClick>
            <div
                className={cn(
                    "workspace-rail-item group relative mx-1.5 flex h-9 cursor-pointer items-center rounded-md transition-colors select-none",
...
const RailButton = memo(({ icon, label, compact, onClick }: RailButtonProps) => {
    return (
...
export const WorkspaceRail = memo(() => {
    const env = useWaveEnv<WorkspaceRailEnv>();
    const railModel = WorkspaceRailModel.getInstance();
    const compact = useAtomValue(railModel.compactAtom);
    ...
    return (
        <div
            className="flex h-full flex-col overflow-hidden border-r border-border/40"
        ...
        >
            ...
            <div className="flex shrink-0 flex-col gap-0.5 pb-1.5 pt-1">
                ...
                <RailButton
                    ...
                    compact={compact}
                    onClick={() => railModel.toggleCompact()}
                />
            </div>
        </div>
    );
