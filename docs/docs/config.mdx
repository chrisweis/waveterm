...
| app:workspacesidebar                                                    | bool     | Set to true to replace the workspace switcher button with a persistent workspace sidebar down the left edge of the window (defaults to false). Toggle compact/expanded from the chevron at the bottom of the sidebar                                              |
| app:activityglow                                                        | bool     | Set to true to tint recently-used tabs and workspaces, brightest for the most recent and fading as they go unused, so you can see where you were last working (defaults to false)                                                                                |
| tab:autosize                                                            | bool     | Set to true to size each tab to the width of its own name instead of giving every tab the same width; tabs that overflow the bar scroll as usual (defaults to false)                                                                                              |
